...
RESULTS      = 'kick-tires-stdout.txt'                                      # Output file with synthesis results
TIMERESULTS  = 'kick-tires-timings.txt'
FINALRESULTS = 'kick-tires.txt'

SYNQUID_ROOT = './synquid'
HOOGLE_ROOT = './hoogle_plus'
...
def run_benchmark_variants(file, variant):
  ...
  with open(RESULTS, "a") as outfile:
    ...
    if variant == 'hegel':
        ...
        usage_start = resource.getrusage(resource.RUSAGE_CHILDREN)
        try:
            run(['/usr/bin/time', prudent, '-cdcl', file], timeout =TIMEOUT,  stdout=outfile)
            usage_end = resource.getrusage(resource.RUSAGE_CHILDREN)    
            cpu_time = usage_end.ru_utime - usage_start.ru_utime    
        except TimeoutExpired:
            cpu_time = 1000        
        ...
        usage_start = resource.getrusage(resource.RUSAGE_CHILDREN)
        try:
            run(['/usr/bin/time', prudent, '-bi', file], timeout =TIMEOUT,  stdout=outfile)
            usage_end = resource.getrusage(resource.RUSAGE_CHILDREN)    
            cpu_time = usage_end.ru_utime - usage_start.ru_utime    
        except TimeoutExpired:
            cpu_time = 1000        
        ...
        usage_start = resource.getrusage(resource.RUSAGE_CHILDREN)
        try:
            run(['/usr/bin/time', prudent, file], timeout =TIMEOUT,  stdout=outfile)
            usage_end = resource.getrusage(resource.RUSAGE_CHILDREN)    
            cpu_time = usage_end.ru_utime - usage_start.ru_utime    
        except TimeoutExpired:
            cpu_time = 1000        
        ...
        usage_start = resource.getrusage(resource.RUSAGE_CHILDREN)
        try:
            run(['/usr/bin/time', prudent, '-bi', '-cdcl', '-k', '3', file], timeout =TIMEOUT,  stdout=outfile)
            usage_end = resource.getrusage(resource.RUSAGE_CHILDREN)    
            cpu_time = usage_end.ru_utime - usage_start.ru_utime    
        except TimeoutExpired:
            cpu_time = 1000        
...
def run_benchmark_synquid(file):
  ...
  with open(RESULTS, "a") as outfile:
    ...
    usage_start = resource.getrusage(resource.RUSAGE_CHILDREN)
    try:
        run(['/usr/bin/time', 'stack',  'exec',  '--', 'synquid', file], 
        cwd = SYNQUID_ROOT,
        timeout =TIMEOUT,  
        stdout=outfile)
        usage_end = resource.getrusage(resource.RUSAGE_CHILDREN)    
